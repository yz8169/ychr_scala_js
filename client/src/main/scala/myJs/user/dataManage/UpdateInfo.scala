...
/**
  ...
  */
@JSExportTopLevel("UpdateInfo")
object UpdateInfo {

  val number = $("input[name='number']").`val`()
  val parentStr = "#updateInfoModal"
    ...
    val ajaxSettings = JQueryAjaxSettings.url(s"${url}?number=${number}").`type`("get").
      success { (data, status, e) =>
        ...
        val kitNameHtml = if (kitName != "") {
          ...
          select(cls := "form-control", name := "snpKitName", borderRadius := 4, onchange := "UpdateInfo.snpChange(this)",
            option(value := "", "---请选择---"),
            raw(optionHtml),
          ).render
        }
    ...
    val ajaxSettings = JQueryAjaxSettings.url(s"${url}?number=${number}").`type`("get").
      success { (data, status, e) =>
        ...
        val kitNameHtml = if (kitName != "") {
          ...
          select(cls := "form-control", name := "strKitName", borderRadius := 4, onchange := "UpdateInfo.change(this)",
            option(value := "", "---请选择---"),
            option(value := "QuickLine", "QuickLine"),
            option(value := "PathFinder Plus", "PathFinder Plus"),
          ).render
        }
        $(s"${parentStr} #strKitName").html(kitNameHtml)
        val html = if (b) {
          val myV = rs.myGet("value")
          val row = JSON.parse(myV).asInstanceOf[js.Dictionary[String]]
  ...
  def snpChange(y: Element) = {
    val value = $(y).find(">option:selected").`val`()
    refreshSnp(parentStr,value.toString)

  }

  def refreshSnp(parentStr:String,key: String) = {
    val html = if (key != "") {
      Shared.snpMap(key).map { v =>
        div(cls := "form-group",
          div(cls := "myLabel", v),
          input(hidden, name := "snpSiteNames[]", value := v),
          input(`type` := "text", id := "rg-from", name := "snpSiteDatas[]", value := "", cls := "form-control")
        )
      }.mkString("&nbsp;")
    } else ""
    $(s"${parentStr} #snp").html(html)
